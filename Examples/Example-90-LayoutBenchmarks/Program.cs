...
Run("deep-fit-256", BuildDeep(256));
Run("fit-nesting-1111", BuildFitNesting());
...
static void RunCachedRead(string name, Fixture fixture)
{
    ...
    for (var i = 0; i < iterations; i++) checksum += fixture.Root.Rect.W;
    stopwatch.Stop();
...
static Fixture BuildFitNesting()
{
    var gui = new BenchmarkGui(1_000, 1_000);
    var root = LayoutNode.CreateRoot(gui, 1_000, 1_000);
    var nodes = 1;
    for (var a = 0; a < 10; a++)
    {
        ...
        for (var b = 0; b < 10; b++)
        {
            ...
            for (var c = 0; c < 10; c++)
            {
                second.AddChild(new LayoutNode($"c{a}-{b}-{c}", gui, second, 10f, 10f));
                nodes++;
            }
        }
    }
...
{
    public void Layout() => Root.CalculateLayout();
}
